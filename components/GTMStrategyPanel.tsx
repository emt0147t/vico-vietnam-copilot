...
const LOADING_PHASES = [
  ...
  "Tạo Market Reports từ GSO, Bộ TT&TT...",
  "Chạy Scenario Modeling...",
  "Hoàn thiện Living Playbook...",
];

const VALUE_METRICS = [
  { label: "Nhanh hơn", value: "5x", sub: "so với nghiên cứu thủ công", color: "from-blue-500 to-cyan-400" },
  { label: "Độ chính xác", value: "95%", sub: "xác minh từ nguồn uy tín", color: "from-emerald-500 to-green-400" },
  { label: "Tiết kiệm chi phí", value: "87%", sub: "so với thuê consulting", color: "from-violet-500 to-purple-400" },
  { label: "Nguồn dữ liệu", value: "12+", sub: "GSO, Bộ TT&TT, Statista...", color: "from-amber-500 to-orange-400" },
];
...
const FEATURE_MODULES = [
  ...
  },
  {
    ...
    features: ["GSO data", "Bộ TT&TT", "Statista", "VECITA"],
  },
  {
    icon: "🧪",
...
export function GTMStrategyPanel() {
  ...
  return (
    <div className="space-y-6">
      {/* ═══ HERO HEADER ═══ */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 border border-gray-700/50 p-8">
        {/* Background pattern */}
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-2 mb-4">
            <span className="px-3 py-1 bg-red-500/20 border border-red-500/30 rounded-full text-red-400 text-xs font-semibold tracking-wider uppercase">
              Global Copilot
            </span>
            <span className="px-3 py-1 bg-blue-500/20 border border-blue-500/30 rounded-full text-blue-400 text-xs font-semibold tracking-wider uppercase">
              Living Playbook
            </span>
            <span className="px-3 py-1 bg-emerald-500/20 border border-emerald-500/30 rounded-full text-emerald-400 text-xs font-semibold tracking-wider uppercase">
              Vietnam Market
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {VALUE_METRICS.map((m, i) => (
              <div key={i} className="bg-gray-800/60 backdrop-blur border border-gray-700/50 rounded-xl p-4 text-center group hover:border-gray-600 transition-all">
      ...
      <div className="bg-gray-800/50 backdrop-blur border border-gray-700/50 rounded-xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-white font-semibold">Tạo Living Playbook</h3>
            <p className="text-gray-400 text-sm">Nhập tên công ty để AI xây dựng chiến lược GTM toàn diện</p>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3.5 border border-gray-600 rounded-xl bg-gray-900/80 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-red-500/50 transition-all"
            />

            {showSuggestions && companies.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-gray-800 border border-gray-600 rounded-xl shadow-2xl z-10 max-h-48 overflow-y-auto">
                {companies.map((company, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleSelectCompany(company)}
                    className="w-full text-left px-4 py-3 hover:bg-gray-700 text-white first:rounded-t-xl last:rounded-b-xl transition-colors flex items-center gap-3"
                  >
        ...
        {state.loading && (
          <div className="mt-4 bg-gray-900/50 rounded-lg p-4 border border-gray-700/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white text-sm font-medium">{state.loadingPhase}</p>
      ...
      {state.error && (
        <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-red-300">Lỗi tạo Playbook</h3>
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {FEATURE_MODULES.map((module, i) => (
              <div
                key={i}
                className="bg-gray-800/40 border border-gray-700/50 rounded-xl p-6 hover:border-gray-600 transition-all group"
              >
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-gray-700/50 rounded-xl flex items-center justify-center text-2xl group-hover:scale-110 transition-transform">
                    {module.icon}
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {module.features.map((f, j) => (
                        <span
                          key={j}
                          className="px-2 py-0.5 bg-gray-700/50 text-gray-400 text-xs rounded-md border border-gray-700"
                        >
                          {f}
                        </span>
                      ))}
          ...
          <div className="bg-gray-800/30 border border-gray-700/30 rounded-xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-emerald-400 text-sm font-semibold">Nguồn dữ liệu xác minh</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {[
                { name: "Tổng cục Thống kê (GSO)", flag: "🇻🇳" },
                { name: "Bộ TT&TT", flag: "🇻🇳" },
                { name: "VECITA", flag: "🇻🇳" },
                { name: "Statista", flag: "🌍" },
                { name: "World Bank", flag: "🌍" },
                { name: "Bloomberg", flag: "🌍" },
                { name: "VCCI", flag: "🇻🇳" },
                { name: "Bộ Kế hoạch & Đầu tư", flag: "🇻🇳" },
              ].map((src, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-800/80 border border-gray-700/50 rounded-lg text-gray-300 text-xs"
                >
                  <span>{src.flag}</span>
                  {src.name}
                </span>
            ...
            <p className="text-gray-500 text-sm">
              ...
              {["Vingroup", "FPT Software", "Techcombank", "VietJet", "Viettel"].map((c, i) => (
                <button
                  ...
                  className="text-red-400 hover:text-red-300 transition-colors mx-1 underline underline-offset-2"
                >
                  {c}
                </button>
              ))}
